...
const KINDS = ['entity', 'decision', 'convention', 'note'];
const MAX_LIMIT = 50;
...
function renderRows(outcome, heading) {
...
function queryOptions(kind, scope, tags, limit) {
    return {
        ...(kind === undefined ? {} : { kind }),
...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [textBlock(`${value.created ? 'Stored' : 'Updated'} memory ${value.id} (${value.kind}: ${value.subject})`)],
        },
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (exec.agent === undefined)
                throw new Error('memory_recall requires an owning agent session');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (exec.agent === undefined)
                throw new Error('memory_list requires an owning agent session');
