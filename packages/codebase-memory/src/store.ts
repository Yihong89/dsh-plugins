...
import { FILE_VERSION, type MemoryEntry, type MemoryFile, type MemoryKind, type MemorySearchRow } from './types.js'
...
const EMPTY_FILE: MemoryFile = { version: FILE_VERSION, entries: [] }
...
function scoreOf(entry: MemoryEntry, queryTokens: Set<string>): number {
  let score = 0
  ...
  for (const token of queryTokens) {
    if (subjectTokens.has(token)) score += 3
    if (tagTokens.has(token)) score += 2
    if (scopeTokens.has(token)) score += 2
    if (contentTokens.has(token)) score += 1
    if (entry.subject.toLowerCase().includes(token)) score += 1
  }
  return score
}

function toRow(entry: MemoryEntry): MemorySearchRow {
  return {
    id: entry.id,
    kind: entry.kind,
...
export class MemoryStore {
  ...
  private refresh(): void {
    ...
    if (this.loadedStat !== null
      ...
      && stat.size === this.loadedStat.size) return
   ...
   */
  remove(target: { id?: string; kind?: MemoryKind; subject?: string; scope?: string }): Promise<number> {
    return this.mutate(() => {
      this.refresh()
      const before = this.entries.length
      this.entries = this.entries.filter(entry => {
        if (target.id !== undefined && entry.id === target.id) return false
        if (target.subject !== undefined
          && entry.subject === target.subject
          && (target.kind === undefined || entry.kind === target.kind)
          && (target.scope === undefined || (entry.scope ?? undefined) === target.scope)) return false
        return true
      })
...
function mergeTags(existing: string[] | undefined, next: string[] | undefined): string[] | undefined {
 ...
 */
export function memoryFilePath(workspaceDir: string, dirName: string, fileName: string): string {
