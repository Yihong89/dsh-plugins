...
function parseFlat(text) {
    const m = text.match(/1M INPUT TOKENS \(CACHE HIT\) \$(\S+) \$(\S+)[\s\S]*?1M INPUT TOKENS \(CACHE MISS\) \$(\S+) \$(\S+)[\s\S]*?1M OUTPUT TOKENS \$(\S+) \$(\S+)/);
    if (m === null)
        return undefined;
    const [hitF, hitP, missF, missP, outF, outP] = m.slice(1).map(parseFloat);
    if (![hitF, hitP, missF, missP, outF, outP].every((n) => Number.isFinite(n)))
        return undefined;
    return {
...
function parsePeakOffpeak(text) {
    const m = text.match(/deepseek-v4-flash OFF-PEAK \$(\S+) \$(\S+) \$(\S+) PEAK \$(\S+) \$(\S+) \$(\S+)[\s\S]*?deepseek-v4-pro OFF-PEAK \$(\S+) \$(\S+) \$(\S+) PEAK \$(\S+) \$(\S+) \$(\S+)/);
    if (m === null)
        return undefined;
    ...
    const windows = [[1 * 60, 4 * 60], [6 * 60, 10 * 60]];
    const effectiveAt = '2026-08-16T16:00:00Z';
    const make = (peak, offPeak) => ({ peak, offPeak, peakWindowsUtc: windows, effectiveAt });
    return {
        flash: make({ inputPerMillion: pkFm, cacheReadPerMillion: pkFh, outputPerMillion: pkFo }, { inputPerMillion: offFm, cacheReadPerMillion: offFh, outputPerMillion: offFo }),
        pro: make({ inputPerMillion: pkPm, cacheReadPerMillion: pkPh, outputPerMillion: pkPo }, { inputPerMillion: offPm, cacheReadPerMillion: offPh, outputPerMillion: offPo }),
    };
}
/**
 ...
 */
export async function fetchDeepseekPrices(url) {
    try {
        ...
        if (flat === undefined || peakOffpeak === undefined)
            return undefined;
 ...
 */
export function diffPrices(local, parsed) {
    const lines = [];
...
//# sourceMappingURL=price-check.js.map