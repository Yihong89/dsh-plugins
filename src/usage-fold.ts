...
import { costOf, priceFor, type PricingMode, type PriceTable, type UsageBuckets } from './pricing.js'
...
function usageOf(event: SessionEvent): { turn: number; step: number; buckets: UsageBuckets; timeMs: number } | undefined {
...
const bucketKeys: readonly (keyof UsageBuckets)[] = [
  'uncachedInputTokens',
  'cacheReadTokens',
  'cacheWriteTokens',
  'outputTokens',
...
function addBuckets(base: ModelUsage, next: UsageBuckets, requests: number, cost: number): ModelUsage {
  const result: ModelUsage = { ...base, requests, cost }
  for (const key of bucketKeys) result[key] = base[key] + next[key]
  return result
}

/** Subtract `removed` bucket values from `base`, returning a new object. */
function subtractBuckets(base: ModelUsage, removed: UsageBuckets, requests: number, cost: number): ModelUsage {
  const result: ModelUsage = { ...base, requests, cost }
 ...
 */
export function applyUsage(state: UsageReportState, event: SessionEvent, options: FoldOptions): UsageReportState {
...
export function viewUsage(state: UsageReportState): UsageReportValue {
  ...
  for (const [model, usage] of Object.entries(state.models)) {
    if (usage.requests === 0 && usage.cost === 0 && usage.uncachedInputTokens === 0
      && usage.cacheReadTokens === 0 && usage.cacheWriteTokens === 0 && usage.outputTokens === 0) continue
    models[model] = usage
 ...
 */
export function usageReportProjectionWith(options: FoldOptions): ProjectionDefinition<'usageReport', UsageReportState> {
  return {
    key: 'usageReport',
